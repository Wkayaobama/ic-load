...
| Repomix narrow schema bundle | user requirement | Keep | Yes | Review/generation context |
| Gomplate SQL-only templating | user requirement | Keep | Yes | Avoids templating the wrong layers |
...
There are 20 capabilities required for the 85% threshold.
...
5. gold upsert SQL execution path
6. association bridge execution path
7. clean devcontainer/Codespaces flow
8. smoke tests and operator docs
...
- package SQL upsert and association execution path
...
4. run the core Bronze -> Silver -> dbt -> upsert -> association path
5. understand failures from one operator-facing runbook
