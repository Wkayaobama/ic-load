#!/usr/bin/env python3
"""
validate_silver.py (standalone version for ic-load)
====================================================
...
This is a standalone version adapted from the parent IC_Load repo
to use ic-load's context.db module for PostgreSQL connectivity.
"""
...
class CheckResult:
    ...
    def __str__(self) -> str:
        icon = "OK" if self.passed else ("FAIL" if self.severity == "STOP" else "WARN")
...
class SilverValidator:
    ...
    def check_opportunity(self) -> None:
        ...
        # Amount unit sanity: avg forecast > 50,000 suggests absolute euros not k euros
        ...
        self._add("opportunity.avg_forecast", "STOP" if likely_absolute else "INFO",
                  round(avg_forecast, 0), "<50,000 (k euros expected)",
                  not likely_absolute,
                  "Average forecast > 50k suggests values are in absolute euros, not k euros. Divide by 1000." if likely_absolute else "")
